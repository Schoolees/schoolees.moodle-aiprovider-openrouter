...
class process_generate_image extends abstract_processor {
    ...
    protected function get_endpoint(): UriInterface {
        return new Uri((string) $this->get_action_setting('endpoint', 'https://api.openai.com/v1/images/generations'));
    }

    #[\Override]
    protected function get_model(): string {
        return (string) $this->get_action_setting('model', 'dall-e-3');
    }
    ...
    protected function handle_api_success(ResponseInterface $response): array {
        ...
        if (!is_object($bodyobj) || empty($bodyobj->data) || empty($bodyobj->data[0]->url)) {
            return [
                'success' => false,
                'errorcode' => -1,
